...
class LDAPService:
    ...
    async def block_user(self, unique_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not current_dn:
                try:
                    current_dn = user.entry_dn  # иногда доступно как свойство
                except Exception:
                    pass
            if not current_dn:
            ...
            if not current_dn and getattr(conn, 'response', None):
                try:
                    ...
                    pass
            if not current_dn:
                # Финальный фоллбэк: отдельный поиск по sAMAccountName
                try:
                    conn.search(
                        'DC=central,DC=st-ing,DC=com',
            ...
            # Удаляем из групп ТОЛЬКО если знаем DN (API ожидает DN участника)
