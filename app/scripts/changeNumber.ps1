...
function Update-ADUserPhoneNumber {
    ...
    Write-Host "New phone added $newTelephoneNumber in AD..."
    $user = Get-ADUser -Filter "pager -eq '$pager'" -Properties pager

    if ($user) {
        $sAMAccountName = $user.sAMAccountName
        Write-Host "User found: $sAMAccountName"
        try {
            Set-ADUser -Identity $sAMAccountName -Replace @{telephoneNumber=$newTelephoneNumber}
...
Update-ADUserPhoneNumber -pager $pager -newTelephoneNumber $newTelephoneNumber
